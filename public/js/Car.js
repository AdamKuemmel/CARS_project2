...
const postCar = async (event) => {
    ...
    console.log(carMileage + carMake + carModel + carYear + carColor + carPrice + carNew + Img)

    console.log(JSON.stringify({carMake, carModel, carYear, carColor, carMileage, carPrice, carNew, Img}))

    if(carMake && carModel && carYear && carColor && carMileage && carPrice){
        const response = await fetch("/api/cars", {
            method: "Post",
            body: JSON.stringify({carMake, carModel, carYear, carColor, carMileage, carPrice, carNew , Img}),
